...
- 記事frontmatterの `kazamiIntro` に、その記事の読者へ向けた風見先生の一言（全角60〜90字）を必ず書きます。未設定だとカテゴリ別の汎用文にフォールバックしますが、これは書き忘れの保険であり、通常は記事ごとに書き分けます。文の作り方は DESIGN_TONE.md を参照してください。
- 「触らない場所」を箇条書きで並べる構成は使いません。読者はもともと分解しないため、代わりに「①〜③で改善しないときに考えられる原因」「その修理に何が必要か（資格・道具）」「どこへ連絡するか」を書きます。
- メーカーの連絡先は `src/data/makerContacts.ts` に集約し、`/maker-contacts/` から案内します。電話番号は改廃があるためデータに直接書かず、公式ページへ送ります。公式ページで確認できた番号だけ `tel` に入れます。
- 賃貸・社宅の読者向けに、メーカーより先に管理会社・大家へ連絡する順番を必ず添えます。
