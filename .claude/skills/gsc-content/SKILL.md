...
- `kazamiIntro`: **必須**。その記事の読者に風見先生がかける一言(全角60〜90字)。
  「共感 → 決めつけない → 次の一歩」の順で書く。どの記事にも当てはまる汎用文は書かない。
  例(水漏れ): 「床が濡れていくのを見ると、焦りますよね。まずは電源まわりから水を遠ざけて、
  タオルを1枚。原因さがしは、そのあとで大丈夫です。」書き方は `DESIGN_TONE.md` を参照。
- `pubDate` は本日の日付。

本文の構成(既存記事に倣う):
1. 安全に確認できる順序(設定→外側→… の番号付き手順)
2. 「①〜③で直らないときは」: 考えられる原因ごとに、症状 → 直すのに何が必要か
   (資格・道具) → なぜ初心者では無理か、の順で書く。
3. どこに連絡するか(賃貸は管理会社・大家が先、持ち家はメーカー窓口)。
   `/maker-contacts/` と `/articles/call-contractor/` へリンクする。
4. 危険サインで運転を止める案内、必要なら買い替え検討への導線

「🚫 触らない場所」のように触ってはいけない部品を箇条書きで並べる構成は使わない。
読者はもともと分解しないため、そのスペースは上記2〜3に充てる。
...
- `kazamiIntro` が無い、または汎用的な文になっている場合は、その記事の読者の状況に合わせて書き直す。
- 「🚫 触らない場所」形式の箇条書きが残っていれば、「①〜③で直らないときは」の構成に置き換える。
